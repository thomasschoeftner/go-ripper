...
func TestValidateConfig(t *testing.T) {
	...
	t.Run("check for empty workDir", func (t* testing.T) {
		c := &AppConf{WorkDirectory: "", MetaInfoRepo: "x/y/z", DefaultOutputDirectory: "/k/l/m"}
	...
	t.Run("check for empty metaInfoRepo", func (t* testing.T) {
		c := &AppConf{WorkDirectory: "a/b/c", MetaInfoRepo: "", DefaultOutputDirectory: "/k/l/m"}
		test.AssertOn(t).ExpectError("expected error when validating empty metaInfoRepo, but got none")(validateConfig(c))
	})

	t.Run("check for empty metaInfoRepo", func (t* testing.T) {
		c := &AppConf{WorkDirectory: "a/b/c", MetaInfoRepo: "x/y/z", DefaultOutputDirectory: ""}
		test.AssertOn(t).ExpectError("expected error when validating empty metaInfoRepo, but got none")(validateConfig(c))
	})

	t.Run("validate no spaces in workdir", func(t *testing.T) {
		c := &AppConf{WorkDirectory: "a/b c/d", MetaInfoRepo: "x/y/z", DefaultOutputDirectory: "/k/l/m"}
	...
	t.Run("validate no spaces in repodir", func(t *testing.T) {
		c := &AppConf{WorkDirectory: "a/b/c", MetaInfoRepo: "x/y y/z", DefaultOutputDirectory: "/k/l/m"}
	...
	t.Run("validate no spaces in defaultoutputdir", func(t *testing.T) {
		c := &AppConf{WorkDirectory: "a/b/c", MetaInfoRepo: "x/y/z", DefaultOutputDirectory: "/k/l l/m"}
	...
	t.Run("remove leading & trailing spaces in workDir, metaInfoRepo, defaultOutputDir", func(t *testing.T) {
		...
		c := &AppConf{WorkDirectory: " a/b/c  ", MetaInfoRepo: " x/y/z  ", DefaultOutputDirectory: "  /k/l/m "}
		assert.NotError(validateConfig(c))
		assert.StringsEqual("a/b/c", c.WorkDirectory)
		assert.StringsEqual("x/y/z", c.MetaInfoRepo)
		assert.StringsEqual("/k/l/m", c.DefaultOutputDirectory)
	})
